...
public class SnomedRf2ImportService implements ISnomedRf2ImportService {
	...
	private SnomedImportResult doImport(final String version, final ISnomedImportConfiguration configuration, final File archiveFile) throws Exception {
		SnomedImportConfiguration configurationImpl = (SnomedImportConfiguration) configuration;
		String taskId = configurationImpl.getTaskId();
		IBranchPath branchPath;
		if (taskId != null) {
			branchPath = createStorageRef(version, taskId).getBranchPath();
		} else {
			branchPath = createVersionPath(version);
		}
