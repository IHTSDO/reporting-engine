...
public class InactivationImpactAssessment extends AllKnownTemplates implements ReportClass {
	...
	private void checkDerivativeUsage() throws TermServerScriptException {
		...
		for (DerivativeLocation location : derivativeLocationMap.values()) {
			try {
				checkInactivationsForDerivative(location, serverTsClientMap, serverAsClientMap, branchForCodeSystemMap);
	...
	private void checkInactivationsForDerivative(DerivativeLocation location, Map<String,TermServerClient> serverTsClientMap, Map<String,AuthoringServicesClient> serverAsClientMap, Map<String, String> branchForCodeSystemMap) throws TermServerScriptException {
		...
		String branch = branchForCodeSystemMap.computeIfAbsent(location.codeSystem, k -> getBranchForDerivativeLocation(tsClient, asClient, location));
	...
	private String getBranchForDerivativeLocation(TermServerClient tsClient, AuthoringServicesClient asClient, DerivativeLocation location) {
		...
		try {
			...
			if (location.project != null) {
				...
				//For projects we need to work with authoring-services
				return asClient.getProject(location.project).getBranchPath();
			}
