			...
			if (comments != null && !comments.isEmpty()) {
				...
				if (requestAlreadyHasComments(requestId, conn)) {
					String message = String.format(
							"CRS request %d (Jira %s) already has comments. Skipping.",
							requestId, task.getKey()
					);
					LOGGER.warn(message);
					report(SECONDARY_REPORT,
							task.getKey(),
							...
							Severity.HIGH,
							ReportActionType.VALIDATION_CHECK);
					incrementSummaryInformation("CRS request already has comments");
					return;
				}
