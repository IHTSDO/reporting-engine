...
public class SnomedUtils extends SnomedUtilsBase implements ScriptConstants {
	...
	public static boolean hasUsGbVariance(Description d) {
		if (d.isAcceptable(US_ENG_LANG_REFSET) && !d.isAcceptable(GB_ENG_LANG_REFSET)) {
			return true;
		}
		if (d.isPreferred(US_ENG_LANG_REFSET) && !d.isPreferred(GB_ENG_LANG_REFSET)) {
			return true;
		}
		if (d.isAcceptable(GB_ENG_LANG_REFSET) && !d.isAcceptable(US_ENG_LANG_REFSET)) {
			return true;
		}
		if (d.isPreferred(GB_ENG_LANG_REFSET) && !d.isPreferred(US_ENG_LANG_REFSET)) {
			return true;
		}
		return false;
	}
