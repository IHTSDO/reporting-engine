...
public class CdoStore {
	...
	private static final String getConceptsChangedSince = 
		" select concept_sctid, max(max_cdo_created) from ( "
		+ " select id as concept_sctid, max(cdo_created) as max_cdo_created "
		+ " from snomed_concept "
		+ " where cdo_branch = ? "
		+ " group by concept_sctid "
		+ " union "
		+ " select c.id, max(d.cdo_created) "
		...
		+ " and crl.cdo_branch = ? "
		+ " and c.cdo_branch = crl.cdo_branch "
		+ " group by c.id ) all_components " 
		+ " where cdo_created >= ?  "
		+ " group by concept_sctid";
